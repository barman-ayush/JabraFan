  ...
  const getTeamScore = (periodScores: any[], isHomeTeam: boolean) => {
    ...
    return isHomeTeam 
      ? {
          ...
          wickets: firstInnings?.home_wickets ?? secondInnings?.home_wickets ?? 0,
          batting: currentInning === 2 && secondInnings?.home_score >= 0
        }
      : {
          score: firstInnings?.away_score || secondInnings?.away_score || 0,
          wickets: firstInnings?.away_wickets ?? secondInnings?.away_wickets ?? 0,
          batting: currentInning === 2 && secondInnings?.away_score >= 0
        };
  };

  const homeTeamScore = getTeamScore(match.data.sport_event_status.period_scores, true);
  const awayTeamScore = getTeamScore(match.data.sport_event_status.period_scores, false);
      ...
      <div className="mt-4 sm:mt-8 flex items-center justify-between px-2 sm:px-4">
        ...
        <div className="text-center flex-1">
          <div className="relative w-20 h-20 sm:w-32 sm:h-32 mx-auto mb-2 sm:mb-4">
            <Image
              src={
                "https://www.bing.com/images/search?q=generic%20team%20image&FORM=IQFRBA&id=D2E0A58039BC41CB399655A34F1C905284F82885"
              }
