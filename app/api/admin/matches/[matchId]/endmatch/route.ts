...
export async function GET(request: NextRequest, { params }: { params: any }) {
  try {
    ...
    if (!updatedMatch) {
      return NextResponse.json(
        { error: "Cannot Update Match" },
        { status: 404 }
      );
    }

    // Get prize money amounts from environment variables
    const firstPrize = process.env.FIRST_PRIZE
      ? parseInt(process.env.FIRST_PRIZE)
      : 500;
    const secondPrize = process.env.SECOND_PRIZE
      ? parseInt(process.env.SECOND_PRIZE)
      : 300;
    const thirdPrize = process.env.THIRD_PRIZE
      ? parseInt(process.env.THIRD_PRIZE)
      : 200;
    ...
    if (topUsers.length > 0) {
      ...
      for (let i = 0; i < Math.min(topUsers.length, 3); i++) {
        ...
        console.log(
          `Awarded ₹${prizes[i].amount} to user ${
            topUsers[i].userId
          } for position ${i + 1}`
        );
      }
